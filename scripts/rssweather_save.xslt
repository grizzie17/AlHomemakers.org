...
<xsl:template match="yweather:condition">
	<div>
	<img height="26" width="26" align="texttop"><xsl:attribute name="alt"><xsl:value-of select="@text"/></xsl:attribute><xsl:attribute name="src">images/weather/<xsl:value-of select="@code"/>.gif</xsl:attribute></img>
	<span class="weatherCurrentTemp"><xsl:value-of select="@temp"/>&#xB0;</span>
	<xsl:text> </xsl:text>
...
<xsl:template match="yweather:forecast">
	...
	<xsl:text> </xsl:text>
	<span class="weatherForecastHigh"><xsl:value-of select="@high"/><xsl:if test="@high != '-'">&#xB0;</xsl:if></span> / <span class="weatherForecastLow"><xsl:value-of select="@low"/><xsl:if test="@low != '-'">&#xB0;</xsl:if></span>
	</div>
</xsl:template>
