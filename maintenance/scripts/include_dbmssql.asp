...
SUB loadUserConfig()
	DIM	sPath
	sPath = findFileUpTree( "config\configuser.asp" )
	IF 0 < LEN(sPath) THEN
		DIM	oFile
...
FUNCTION dbConnectAuth( sDBName, sUser, sPassword )
	...
	IF NOT Nothing IS oDC THEN
		DIM	sConn
		IF "" <> g_sDBConnectString THEN
			sConn = g_sDBConnectString
		ELSE
			IF g_bDBDSN THEN
				sConn = "DSN=grizzie_" & sDBName & ";" _
					&	"UID=" & sUser & ";" _
					&	"Pwd=" & sPassword & ";" _
					&	"DATABASE=grizzie_" & sDBName & ";"
				sConn = "" _
					&	"Provider=MSDASQL;" _
					&	"PersistSecurityInfo=False;" _
					&	"UID=" & sUser & ";" _
					&	"Pwd=" & sPassword & ";" _
					&	"Data Source=grizzie_" & sDBName & ";" _
					&	"catalog=grizzie_" & sDBName & ";"
			ELSE
				sConn = "" _
					&	"Provider=SQLNCLI11;" _
					&	"SERVER=" & g_sDBServer & ";" _
					&	"Driver={SQL Server};" _
					&	"Integrated Security=SSPI;" _
					&	"Database=grizzie_" & sDBName & ";" _
					&	"UID=(" & sUser & ");" _
					&	"Pwd=(" & sPassword & ");"
				sConn = "" _
		...
		ON ERROR RESUME NEXT
			...
			'oDC.Provider = "SQLOLEDB"
			oDC.open sConn
			DIM	sErr, nErr
			nErr = Err.number
			sErr = Err.Description
		ON ERROR GOTO 0
		IF 0 <> nErr  OR  oDC.state <> adStateOpen THEN
			...
			IF oDC.state <> adStateOpen THEN
				...
				NEXT
			END IF
			Response.Write "<p>" & Server.HTMLEncode(sConn) & "</p>" & vbCRLF
			Response.Flush
		ELSE
			IF oDC.state <> adStateOpen THEN
				SET aErrors = oDC.Errors
