...
FUNCTION htmlFormatWebLinkEnd( iLinkStart, BYREF sText, lenText, cKey )
	...
	END IF
	ix = INSTR( iLinkStart, sText, vbCRLF, vbTextCompare )
	IF 0 < ix  AND  ix < iLinkEnd THEN iLinkEnd = ix
...
SUB htmlFormatFindWEB( iLinkStart, iLinkEnd, sText, sKey, iStart )
	...
	DO
		...
		ELSE
			...
			ix = INSTR( iLinkStart, sText, "{", vbTextCompare )
			IF 0 < ix  AND  ix < iLinkEnd THEN iLinkEnd = ix
			DO WHILE 0 < INSTR(".,!?{}", MID(sText, iLinkEnd-1, 1 ))
				iLinkEnd = iLinkEnd - 1
			LOOP
...
FUNCTION htmlFormatIsBlock( BYREF sLine )
	htmlFormatIsBlock = FALSE
	IF "{{" = LEFT(sLine,2) THEN
		...
		ELSE
			IF ":" = RIGHT(sTemp,1) THEN htmlFormatIsBlock = TRUE
		END IF
		i = INSTR(sTemp,":")
...
FUNCTION htmlFormatSpecialP( BYREF sLine, nExtraBreaks )
	...
	IF 1 < LEN(sLine) THEN
		...
		ELSEIF "=" = sTemp  AND  htmlFormatJustDashes(sLine,"=") THEN
			htmlFormatSpecialP = "{{hr thick}}" & vbCRLF
	...
	ELSE
		IF "|" = sLine THEN
			...
			'htmlFormatSpecialP = "<br>" & vbCRLF
			IF 0 < nExtraBreaks THEN
				htmlFormatSpecialP = htmlFormatSpecialP & "{{br}}" & vbCRLF
				'htmlFormatSpecialP = htmlFormatSpecialP & "<br>" & vbCRLF
			END IF
		ELSEIF "\" = sLine  OR  "/" = sLine THEN
			htmlFormatSpecialP = "{{br clear}}{{br}}" & vbCRLF
...
FUNCTION htmlFormatEncodeP( BYREF aText(), nBegin, nEnd )
	...
	s = ""
	IF nBegin <= nEnd THEN
		DO WHILE htmlFormatIsBlock( aText(nBegin) )
			s = s & aText(nBegin) & vbLF
			nBegin = nBegin + 1
		LOOP
		IF nBegin <= nEnd THEN
	...
	IF nBegin <= nEnd THEN
		...
		FOR n = nBegin TO nEnd
			...
			IF "|" = RIGHT(sTemp,1) THEN
				b = FALSE
				sTemp = LEFT(sTemp,LEN(sTemp)-1) & "}}}}" & vbLF
			...
			ELSE
				...
				END IF
			END IF
			s = s & REPLACE( sTemp, "|", "}}{{td valign=top:" )
...
FUNCTION htmlFormatEncodePindent( BYREF aText(), nBegin, nEnd )
	htmlFormatEncodePindent = htmlFormatEncodePrivateList( aText, nBegin, nEnd, "indent", "", "", "]" )
END FUNCTION
			...
			ELSE
				...
				IF sPrefix = sTemp THEN
					...
					IF j < i THEN
						DO WHILE j < i
							j = j + 1
							s = s & "{{" & sTag & htmlFormatNumberType(sPrefix, j) & ":" '& vbCRLF
...
FUNCTION htmlFormatEncodeLI( BYREF aText(), nBegin, nEnd, sTag, sPrefix )
	htmlFormatEncodeLI = htmlFormatEncodePrivateList( aText, nBegin, nEnd, sTag, "{{li:", "}}", sPrefix )
END FUNCTION
...
FUNCTION htmlFormatMeta_f_args( BYREF sMissing, _
		BYREF sAlign, BYREF sVAlign, BYREF sBorder, BYREF sWidth, BYREF sColor, _
		aArgs(), idx )
	DIM	sTemp
	DIM	sTag
	DIM	sValue
...
FUNCTION htmlFormatMeta_f_sidebar( aArgs(), sText )
	IF 0 < LEN(sText) THEN
		...
		sBorder = ""
		sColor = ""
		IF htmlFormatMeta_f_args( sMissing, sAlign, sVAlign, sBorder, sWidth, sColor, aArgs, 1 ) THEN
...
FUNCTION htmlFormat_f_pullunquoteEllipses( sText )
	htmlFormat_f_pullunquoteEllipses = "{{br}}...{{br}}" & vbLF
END FUNCTION
...
FUNCTION htmlFormatMeta_f_picture( aArgs(), sText )
	...
	sOutput = ""
	IF 0 < UBOUND(aArgs) THEN
		IF htmlFormatMeta_f_args( sMissing, sAlign, sVAlign, sBorder, sWidth, sColor, aArgs, 2 ) THEN
...
FUNCTION htmlFormatMeta_f_align( aArgs(), sText )
	IF 1 = UBOUND(aArgs) THEN
		SELECT CASE LCASE(aArgs(1))
		CASE "left"
			htmlFormatMeta_f_align = "<div style=""text-align: left"">" & htmlFormatMeta(sText) & "</div>"
		CASE "center"
			htmlFormatMeta_f_align = "<div style=""text-align: center"">" & htmlFormatMeta(sText) & "</div>"
		CASE "right"
			htmlFormatMeta_f_align = "<div style=""text-align: right"">" & htmlFormatMeta(sText) & "</div>"
		CASE ELSE
			htmlFormatMeta_f_align = htmlFormatMeta_fail( aArgs, sText )
		END SELECT
	ELSE
		htmlFormatMeta_f_align = htmlFormatMeta_fail( aArgs, sText )
	END IF
...
FUNCTION htmlFormatMeta_f_br( aArgs(), sText )
	IF "" = htmlFormat_blockState() THEN
		IF 0 = LEN(sText) THEN
			IF 1 = UBOUND(aArgs) THEN
				SELECT CASE LCASE(aArgs(1))
				CASE "clear"
					htmlFormatMeta_f_br = "<br clear=""all"">"
				CASE "page"
					htmlFormatMeta_f_br = "<br clear=""all"" class=""htmlformatpagebreak"">"
				CASE ELSE
					htmlFormatMeta_f_br = htmlFormatMeta_fail( aArgs, sText )
				END SELECT
			ELSE
				htmlFormatMeta_f_br = "<br>"
			END IF
		ELSE
			htmlFormatMeta_f_br = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		' just ignore the break
		htmlFormatMeta_f_br = " "
	END IF
END FUNCTION


FUNCTION htmlFormatMeta_f_pagebreak( aArgs(), sText )
	htmlFormatMeta_f_pagebreak = "<!--*--><div style=""page-break-after: always""></div><!--/*-->" & vbCRLF
END FUNCTION


FUNCTION htmlFormatMeta_f_header( aArgs(), sText )
	IF 1 = UBOUND(aArgs) THEN
		IF ISNUMERIC(aArgs(1)) THEN
			DIM	n
			n = CLNG(aArgs(1))
			IF 1 <= n  AND  n <= 6 THEN
				htmlFormatMeta_f_header = "<!--*--><h" & n & " class=""htmlformat"">" & htmlFormatMeta(sText) & "</h" & n & "><!--/*-->" & vbCRLF
			END IF
		ELSE
		END IF
	ELSE
...
DIM	g_aBlockState(30)
DIM	g_nBlockState
g_nBlockState = 0
g_aBlockState(0) = ""

FUNCTION htmlFormat_blockState()
	htmlFormat_blockState = g_aBlockState(g_nBlockState)
END FUNCTION

SUB htmlFormat_blockStatePush( s )
	g_nBlockState = g_nBlockState + 1
	g_aBlockState(g_nBlockState) = LCASE(s)
END SUB

SUB htmlFormat_blockStatePop()
	IF 0 < g_nBlockState THEN
		g_nBlockState = g_nBlockState - 1
	END IF
END SUB


FUNCTION htmlFormatMeta_f_hr( aArgs(), sText )
	IF 1 = UBOUND(aArgs) THEN
		IF "thick" = LCASE(aArgs(1)) THEN
			htmlFormatMeta_f_hr = "<!--*--><hr class=""htmlformat"" style=""height:0.5em""><!--/*-->"
		ELSE
			htmlFormatMeta_f_hr = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		htmlFormatMeta_f_hr = "<!--*--><hr class=""htmlformat""><!--/*-->"
	END IF
END FUNCTION


FUNCTION htmlFormatMeta_f_list( aArgs(), sText )
	IF 1 = UBOUND(aArgs) THEN
		DIM	a
		DIM	sTag
		DIM	c
		...
		IF "type" = LCASE(a(0)) THEN
			...
			CASE ELSE
				...
				sTag = "ol"
			END SELECT
			htmlFormat_blockStatePush "list"
			htmlFormatMeta_f_list = "<!--*--><" & sTag & c & " class=""htmlformat"">" & htmlFormatMeta(sText) & "</" & sTag & "><!--/*-->" & vbCRLF
			htmlFormat_blockStatePop
		END IF
	ELSE
		htmlFormat_blockStatePush "list"
		htmlFormatMeta_f_list = "<!--*--><ul>" & htmlFormatMeta(sText) & "</ul><!--/*-->" & vbCRLF
		htmlFormat_blockStatePop
	END IF
END FUNCTION


FUNCTION htmlFormatMeta_f_listItem( aArgs(), sText )
	htmlFormat_blockStatePush ""
	htmlFormatMeta_f_listItem = "<!--*--><li>" & htmlFormatMeta(sText) & "</li><!--/*-->"
	htmlFormat_blockStatePop
END FUNCTION


FUNCTION htmlFormatMeta_f_para( aArgs(), sText )
	IF 0 < LEN(sText) THEN
		DIM	sMissing
		DIM	sAlign
		DIM	sVAlign
		...
		IF htmlFormatMeta_f_args( sMissing, sAlign, sVAlign, sBorder, sWidth, sColor, aArgs, 1 ) THEN
			DIM	sPara
			sPara = htmlFormatMeta(sText)
			IF "<!--*-->" = LEFT(sPara,8) AND   0 < INSTR(RIGHT(sPara,11),"<!--/*-->") THEN
				htmlFormatMeta_f_para = sPara
			ELSE
				htmlFormatMeta_f_para = "<p" & sAlign & " class=""htmlformat"">" & sPara & "</p>" & vbCRLF
			END IF
		ELSE
			htmlFormatMeta_f_para = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		htmlFormatMeta_f_para = htmlFormatMeta_fail( aArgs, sText )
	END IF
...
FUNCTION htmlFormatMeta_f_indent( aArgs(), sText )
	IF 0 < LEN(sText) THEN
		DIM	sMissing
		DIM	sAlign
		DIM	sVAlign
		...
		IF htmlFormatMeta_f_args( sMissing, sAlign, sVAlign, sBorder, sWidth, sColor, aArgs, 1 ) THEN
			DIM	sPara
			sPara = htmlFormatMeta(sText)
			htmlFormatMeta_f_indent = "<!--*--><ul type=""none"">" & sPara & "</ul><!--/*-->" & vbCRLF
		ELSE
			htmlFormatMeta_f_indent = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		htmlFormatMeta_f_indent = htmlFormatMeta_fail( aArgs, sText )
	END IF
...
FUNCTION htmlFormatMeta_f_table( aArgs(), sText )
	IF 0 < LEN(sText) THEN
		DIM	sMissing
		DIM	sAlign
		DIM	sVAlign
		...
		sColor = ""
		IF htmlFormatMeta_f_args( sMissing, sAlign, sVAlign, sBorder, sWidth, sColor, aArgs, 1 ) THEN
			IF "" = sBorder THEN sBorder = " border=""1"""
			IF "" <> sColor THEN sColor = REPLACE(sColor," color="," bgcolor=" )
			htmlFormat_blockStatePush "table"
			htmlFormatMeta_f_table = "<!--*--><table" & sBorder & sWidth & sAlign & sColor & " cellpadding=""3"" cellspacing=""0"" class=""htmlformattable"">" & htmlFormatMeta(sText) & "</table><!--/*-->" & vbCRLF
			htmlFormat_blockStatePop
		ELSE
			htmlFormatMeta_f_table = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		htmlFormatMeta_f_table = htmlFormatMeta_fail( aArgs, sText )
	END IF
...
FUNCTION htmlFormatMeta_f_tr( aArgs(), sText )
	IF 0 < LEN(sText) THEN
		IF "table" = htmlFormat_blockState() THEN
			htmlFormat_blockStatePush "tablerow"
			htmlFormatMeta_f_tr = "<!--*--><tr>" & htmlFormatMeta(sText) & "</tr><!--/*-->" & vbCRLF
			htmlFormat_blockStatePop
		ELSE
			htmlFormatMeta_f_tr = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		htmlFormatMeta_f_tr = htmlFormatMeta_fail( aArgs, sText )
	END IF
...
FUNCTION htmlFormatMeta_f_td( aArgs(), sText )
	IF "tablerow" = htmlFormat_blockState() THEN
		DIM	sMissing
		DIM	sAlign
		DIM	sVAlign
		DIM	sWidth
		...
		IF htmlFormatMeta_f_args( sMissing, sAlign, sVAlign, sBorder, sWidth, sColor, aArgs, 1 ) THEN
			IF "" <> sColor THEN sColor = REPLACE(sColor," color="," bgcolor=")
			htmlFormat_blockStatePush ""
			htmlFormatMeta_f_td = "<!--*--><td" & sAlign & sVAlign & sWidth & sColor & ">" & htmlFormatMeta(sText) & "</td><!--/*-->"
			htmlFormat_blockStatePop
		ELSE
			htmlFormatMeta_f_td = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		htmlFormatMeta_f_td = htmlFormatMeta_fail( aArgs, sText )
	END IF
...
FUNCTION htmlFormatMeta_f_th( aArgs(), sText )
	IF "table" = htmlFormat_blockState() THEN
		DIM	sMissing
		DIM	sAlign
		DIM	sVAlign
		...
		IF htmlFormatMeta_f_args( sMissing, sAlign, sVAlign, sBorder, sWidth, sColor, aArgs, 1 ) THEN
			IF "" <> sColor THEN sColor = REPLACE(sColor," color="," bgcolor=")
			htmlFormat_blockStatePush ""
			htmlFormatMeta_f_th = "<!--*--><th" & sAlign & sVAlign & sWidth & sColor & ">" & htmlFormatMeta(sText) & "</th><!--/*-->"
			htmlFormat_blockStatePop
		ELSE
			htmlFormatMeta_f_th = htmlFormatMeta_fail( aArgs, sText )
		END IF
	ELSE
		htmlFormatMeta_f_th = htmlFormatMeta_fail( aArgs, sText )
	END IF
...
FUNCTION htmlFormatMeta_f_url( aArgs(), sText )
END FUNCTION
	...
	IF 0 < LEN(sText) THEN
		...
		IF "" <> sTag THEN
			...
			IF "" = htmlFormatMetaSubsFunc THEN
				...
				CASE "p", "para"
					IF "" = htmlFormat_blockState() THEN
						htmlFormatMetaSubsFunc = htmlFormatMeta_f_para( aFormatSplit, sGenText )
					END IF
				CASE "br", "break"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_br( aFormatSplit, sGenText )
				CASE "list"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_list( aFormatSplit, sGenText )
				CASE "li", "*"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_listItem( aFormatSplit, sGenText )
				CASE "h", "header"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_header( aFormatSplit, sGenText )
				CASE "table"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_table( aFormatSplit, sGenText )
				CASE "tr"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_tr( aFormatSplit, sGenText )
				CASE "td"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_td( aFormatSplit, sGenText )
				CASE "th"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_th( aFormatSplit, sGenText )
				CASE "align"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_align( aFormatSplit, sGenText )
				CASE "hr", "rule"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_hr( aFormatSplit, sGenText )
				...
				CASE "url"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_url( aFormatSplit, sGenText )
				CASE "download"
				...
				CASE "code"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_code( aFormatSplit, sGenText )
				CASE "pre"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_pre( aFormatSplit, sGenText )
				CASE "indent"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_indent( aFormatSplit, sGenText )
				CASE "color"
				...
				CASE "pagebreak"
					htmlFormatMetaSubsFunc = htmlFormatMeta_f_pagebreak( aFormatSplit, sGenText )
				CASE "!", "comment"
...
FUNCTION htmlFormat( BYREF sText )
	...
	htmlFormat = ""
	aFormatSplit = SPLIT( sText & vbLF, vbLF, -1, vbTextCompare )
	...
	FOR i = LBOUND(aFormatSplit) TO UBOUND(aFormatSplit)
		...
		CASE "*"
			IF "*" <> sTemp THEN
				IF "" = sLine THEN
					htmlFormat = htmlFormat & htmlFormatEncodeLI( aFormatSplit, j, i-1, "list", "*" )
					state = ""
					j = 5000
				END IF
			ELSE
		...
		CASE ELSE
			...
			ELSEIF "-" = sTemp _
				...
				aFormatSplit(i) = sLine
			ELSEIF "-" = sTemp  AND  htmlFormatJustDashes(sLine,"-") THEN
				htmlFormat = htmlFormat & "{{hr}}" & vbCRLF
				state = ""
				nItemCount = 0
				j = 5000
			ELSEIF "=" = sTemp  AND  htmlFormatJustDashes(sLine,"=") THEN
				htmlFormat = htmlFormat & "{{hr thick}}" & vbCRLF
				state = ""
				nItemCount = 0
				j = 5000
			ELSEIF "#" = sTemp  _
			...
			ELSEIF "|" = sTemp THEN
				...
				j = i
			ELSEIF "\" = sLine THEN
				htmlFormat = htmlFormat & "{{br clear}}" & vbCRLF
				state = ""
				j = 5000
			ELSEIF "\\" = sLine THEN
				htmlFormat = htmlFormat & "{{br page}}" & vbCRLF
				state = ""
				j = 5000
			ELSEIF "/" = sLine THEN
				...
				j = 5000
			ELSEIF "//" = sLine THEN
				htmlFormat = htmlFormat & "{{pagebreak}}" & vbCRLF
				state = ""
				j = 5000
			ELSE
	...
	'Response.Write "<table bgcolor=""#FFCC99"">"
	'Response.Write "<tr>"
	'Response.Write "<td><raw>"
	'Response.Write REPLACE(htmlFormat,vbCRLF,"<br>")
	'Response.Write "</raw></td>"
	'Response.Write "</tr>"
	'Response.Write "</table>"

	sTemp = htmlFormatEncodeText( htmlFormat )
	IF 0 < INSTR(1,sTemp,"{{",vbTextCompare) THEN sTemp = htmlFormatMeta( sTemp )
	htmlFormat = sTemp
