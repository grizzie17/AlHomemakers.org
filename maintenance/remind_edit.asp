...
IF 0 < LEN(sID) THEN
	...
	ELSE
		...
		IF NOT Nothing IS oTemp THEN sTime = oTemp.value
		SET oTemp = oDate.attributes.getNamedItem("duration")
		IF NOT Nothing IS oTemp THEN sDateDuration = oTemp.value
		SET oTemp = oDate.attributes.getNamedItem("duration-view")
		IF NOT Nothing IS oTemp THEN sDateDurationView = oTemp.value
		IF "" = sDateDurationView THEN sDateDurationView = "all"
		SET oTemp = oDate.attributes.getNamedItem("offset")
		IF NOT Nothing IS oTemp THEN sDateOffset = oTemp.value
		SET oTemp = oDate.attributes.getNamedItem("pending")
		IF NOT Nothing IS oTemp THEN sDatePending = oTemp.value
		...
		IF NOT Nothing IS oTemp THEN sContent = oTemp.text
		
		SET oTemp = oEvent.selectSingleNode("location")
		IF NOT Nothing IS oTemp THEN sLocation = oTemp.text
		
		SET oTemp = oEvent.selectSingleNode("comments")
		IF NOT Nothing IS oTemp THEN sComments = oTemp.text
